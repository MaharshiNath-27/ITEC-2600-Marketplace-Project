function [isSuccess, user] = logIn()
    ...
    if ~isempty(userIndex)
        if strcmp(password, passwords{userIndex})
            ...
            isSuccess = logIn();  % Recursive retry
        end
    else
        ...
        if askSignUp == "Y"
            isSuccess = signUp();  % Call signUp if user agrees
        else
            disp('Login failed. Returning to login screen.');
            isSuccess = false;  % Return false instead of recursive call
        end
    end
end
...
function [isSuccess, user] = signUp()
    ...
    email = input('Email: ', 's');
    while not(ismember('@', email)) || not(ismember('.com', email))
